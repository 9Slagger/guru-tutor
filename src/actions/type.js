// USERS_FETCH
export const USERS_FETCH = 'users_fetch'
export const USERS_FETCH_SUCESS = 'users_fetch_sucess'
export const USERS_FETCH_FAILURE = 'users_fetch_fail'
// USERS_CREATE
export const USERS_CREATE = 'users_create'
export const USERS_CREATE_SUCESS = 'users_create_sucess'
export const USERS_CREATE_FAILURE = 'users_create_fail'
// USER_EDIT
export const USER_EDIT = 'user_edit'
export const USER_EDIT_SUCESS = 'user_edit_sucess'
export const USER_EDIT_FAILURE = 'user_edit_fail'
// USER_EDIT_TYPE
export const USER_EDIT_TYPE = 'user_edittype'
export const USER_EDIT_TYPE_SUCESS = 'user_edittype_sucess'
export const USER_EDIT_TYPE_FAILURE = 'user_edittype_fail'
// USER_DELETE
export const USER_DELETE = 'user_delete'
export const USER_DELETE_SUCESS = 'user_delete_sucess'
export const USER_DELETE_FAILURE = 'user_delete_fail'
// ADD_USER_BUY_COURSE
export const ADD_USER_BUY_COURSE = 'ADD_USER_BUY_COURSE'
export const ADD_USER_BUY_COURSE_SUCESS = 'ADD_USER_BUY_COURSE_sucess'
export const ADD_USER_BUY_COURSE_FAILURE = 'ADD_USER_BUY_COURSE_fail'
// LOGIN_AUTH
export const LOGIN_AUTH = 'login_auth'
export const LOGIN_AUTH_SUCESS = 'login_auth_sucess'
export const LOGIN_AUTH_FAILURE = 'login_auth_fail'
// CHECK_AUTH
export const CHECK_AUTH = 'check_auth'
export const CHECK_AUTH_SUCESS = 'check_auth_sucess'
export const CHECK_AUTH_FAILURE = 'check_auth_fail'
// LOGOUT_AUTH
export const LOGOUT_AUTH = 'logout_auth'
// VERIFY_AUTH
export const VERIFY_AUTH = 'Verify_auth'
export const VERIFY_AUTH_SUCESS = 'Verify_auth_sucess'
export const VERIFY_AUTH_FAILURE = 'Verify_auth_fail'
// FETCH_HOME_CONTENT
export const FETCH_HOME_CONTENT = 'fetch_homecontent'
export const FETCH_HOME_CONTENT_SUCESS = 'fetch_homecontent_sucess'
export const FETCH_HOME_CONTENT_FAILURE = 'fetch_homecontent_fail'
// FETCH_HOME_CONTENT_FIRST
export const FETCH_HOME_CONTENT_FIRST = 'fetch_homecontent_FIRST'
export const FETCH_HOME_CONTENT_FIRST_SUCESS = 'fetch_homecontent_FIRST_sucess'
export const FETCH_HOME_CONTENT_FIRST_FAILURE = 'fetch_homecontent_FIRST_fail'
// FETCH_HOME_CONTENT_SECONDE
export const FETCH_HOME_CONTENT_SECONDE = 'fetch_homecontent_SECONDE'
export const FETCH_HOME_CONTENT_SECONDE_SUCESS =
  'fetch_homecontent_SECONDE_sucess'
export const FETCH_HOME_CONTENT_SECONDE_FAILURE =
  'fetch_homecontent_SECONDE_fail'
// FETCH_HOME_CONTENT_THIRD
export const FETCH_HOME_CONTENT_THIRD = 'fetch_homecontent_THIRD'
export const FETCH_HOME_CONTENT_THIRD_SUCESS = 'fetch_homecontent_THIRD_sucess'
export const FETCH_HOME_CONTENT_THIRD_FAILURE = 'fetch_homecontent_THIRD_fail'
// CREATE_HOME_CONTENT_FIRST
export const CREATE_HOME_CONTENT_FIRST = 'create_homecontentfirst'
export const CREATE_HOME_CONTENT_FIRST_SUCESS = 'create_homecontentfirst_sucess'
export const CREATE_HOME_CONTENT_FIRST_FAILURE = 'create_homecontentfirst_fail'
// EDIT_HOME_CONTENT_FIRST
export const EDIT_HOME_CONTENT_FIRST = 'edit_homecontentfirst'
export const EDIT_HOME_CONTENT_FIRST_SUCESS = 'edit_homecontentfirst_sucess'
export const EDIT_HOME_CONTENT_FIRST_FAILURE = 'edit_homecontentfirst_fail'
// EDIT_HOME_CONTENT_SOCOND
export const EDIT_HOME_CONTENT_SOCOND = 'edit_homecontentSOCOND'
export const EDIT_HOME_CONTENT_SOCOND_SUCESS = 'edit_homecontentSOCOND_sucess'
export const EDIT_HOME_CONTENT_SOCOND_FAILURE = 'edit_homecontentSOCOND_fail'
// EDIT_HOME_CONTENT_THIRD
export const EDIT_HOME_CONTENT_THIRD = 'edit_homecontentTHIRD'
export const EDIT_HOME_CONTENT_THIRD_SUCESS = 'edit_homecontentTHIRD_sucess'
export const EDIT_HOME_CONTENT_THIRD_FAILURE = 'edit_homecontentTHIRD_fail'
// DELETE_HOME_CONTENT_FIRST
export const DELETE_HOME_CONTENT_FIRST = 'delete_homecontentfirst'
export const DELETE_HOME_CONTENT_FIRST_SUCESS = 'delete_homecontentfirst_sucess'
export const DELETE_HOME_CONTENT_FIRST_FAILURE = 'delete_homecontentfirst_fail'
// CREATE_HOME_CONTENT_SECOND
export const CREATE_HOME_CONTENT_SECOND = 'create_homecontentsecond'
export const CREATE_HOME_CONTENT_SECOND_SUCESS =
  'create_homecontentsecond_sucess'
export const CREATE_HOME_CONTENT_SECOND_FAILURE =
  'create_homecontentsecond_fail'
// EDIT_HOME_CONTENT_SECOND
export const EDIT_HOME_CONTENT_SECOND = 'edit_homecontentsecond'
export const EDIT_HOME_CONTENT_SECOND_SUCESS = 'edit_homecontentsecond_sucess'
export const EDIT_HOME_CONTENT_SECOND_FAILURE = 'edit_homecontentsecond_fail'
// DELETE_HOME_CONTENT_SECOND
export const DELETE_HOME_CONTENT_SECOND = 'delete_homecontentsecond'
export const DELETE_HOME_CONTENT_SECOND_SUCESS =
  'delete_homecontentsecond_sucess'
export const DELETE_HOME_CONTENT_SECOND_FAILURE =
  'delete_homecontentsecond_fail'
// FETCH_NEW_CONTENT
export const FETCH_NEW_CONTENT = 'fetch_newcontent'
export const FETCH_NEW_CONTENT_SUCESS = 'fetch_newcontent_sucess'
export const FETCH_NEW_CONTENT_FAILURE = 'fetch_newcontent_fail'
// CREATE_NEW_CONTEN
export const CREATE_NEW_CONTEN = 'create_newcontent'
export const CREATE_NEW_CONTEN_SUCESS = 'create_newcontent_sucess'
export const CREATE_NEW_CONTEN_FAILURE = 'create_newcontent_fail'
// DELETE_NEW_CONTEN
export const DELETE_NEW_CONTEN = 'delete_newcontent'
export const DELETE_NEW_CONTEN_SUCESS = 'delete_newcontent_sucess'
export const DELETE_NEW_CONTEN_FAILURE = 'delete_newcontent_fail'
// EDIT_NEW_CONTEN
export const EDIT_NEW_CONTEN = 'edit_newcontent'
export const EDIT_NEW_CONTEN_SUCESS = 'edit_newcontent_sucess'
export const EDIT_NEW_CONTEN_FAILURE = 'edit_newcontent_fail'
// FETCH_PROMOTION_CONTENT
export const FETCH_PROMOTION_CONTENT = 'fetch_promotioncontent'
export const FETCH_PROMOTION_CONTENT_SUCESS = 'fetch_promotioncontent_sucess'
export const FETCH_PROMOTION_CONTENT_FAILURE = 'fetch_promotioncontent_fail'
// CREATE_PROMOTION_CONTEN
export const CREATE_PROMOTION_CONTEN = 'create_promotioncontent'
export const CREATE_PROMOTION_CONTEN_SUCESS = 'create_promotioncontent_sucess'
export const CREATE_PROMOTION_CONTEN_FAILURE = 'create_promotioncontent_fail'
// DELETE_PROMOTION_CONTEN
export const DELETE_PROMOTION_CONTEN = 'delete_promotioncontent'
export const DELETE_PROMOTION_CONTEN_SUCESS = 'delete_promotioncontent_sucess'
export const DELETE_PROMOTION_CONTEN_FAILURE = 'delete_promotioncontent_fail'
// EDIT_PROMOTION_CONTEN
export const EDIT_PROMOTION_CONTEN = 'edit_promotioncontent'
export const EDIT_PROMOTION_CONTEN_SUCESS = 'edit_promotioncontent_sucess'
export const EDIT_PROMOTION_CONTEN_FAILURE = 'edit_promotioncontent_fail'
// FETCH_COURSE
export const FETCH_COURSE = 'FETCH_COURSE'
export const FETCH_COURSE_SUCESS = 'FETCH_COURSE_SUCESS'
export const FETCH_COURSE_FAILURE = 'FETCH_COURSE_FAILURE'
// FETCH_ONE_COURSE
export const FETCH_ONE_COURSE = 'FETCH_ONE_COURSE'
export const FETCH_ONE_COURSE_SUCESS = 'FETCH_ONE_COURSE_SUCESS'
export const FETCH_ONE_COURSE_FAILURE = 'FETCH_ONE_COURSE_FAILURE'
// FETCH_ONE_COURSE_PUBLISH
export const FETCH_ONE_COURSE_PUBLISH = 'FETCH_ONE_COURSE_PUBLISH'
export const FETCH_ONE_COURSE_PUBLISH_SUCESS = 'FETCH_ONE_COURSE_PUBLISH_SUCESS'
export const FETCH_ONE_COURSE_PUBLISH_FAILURE =
  'FETCH_ONE_COURSE_PUBLISH_FAILURE'
// CREATE_COURSE
export const CREATE_COURSE = 'CREATE_COURSE'
export const CREATE_COURSE_SUCESS = 'CREATE_COURSE_SUCESS'
export const CREATE_COURSE_FAILURE = 'CREATE_COURSE_FAILURE'
// EDIT_COURSE
export const EDIT_COURSE = 'EDIT_COURSE'
export const EDIT_COURSE_SUCESS = 'EDIT_COURSE_SUCESS'
export const EDIT_COURSE_FAILURE = 'EDIT_COURSE_FAILURE'
// DELETE_COURSE
export const DELETE_COURSE = 'DELETE_COURSE'
export const DELETE_COURSE_SUCESS = 'DELETE_COURSE_SUCESS'
export const DELETE_COURSE_FAILURE = 'DELETE_COURSE_FAILURE'
// CLEAR_COURSE_ONE
export const CLEAR_COURSE_ONE = 'CLEAR_COURSE_ONE'
// ADD_COURSE_TO_CART
export const ADD_COURSE_TO_CART = 'ADD_COURSE_TO_CART'
export const ADD_COURSE_TO_CART_SUCESS = 'ADD_COURSE_TO_CART_SUCESS'
export const ADD_COURSE_TO_CART_FAILURE = 'ADD_COURSE_TO_CART_FAILURE'
// DELETE_COURSE_TO_CART
export const DELETE_COURSE_TO_CART = 'DELETE_COURSE_TO_CART'
export const DELETE_COURSE_TO_CART_SUCESS = 'DELETE_COURSE_TO_CART_SUCESS'
export const DELETE_COURSE_TO_CART_FAILURE = 'DELETE_COURSE_TO_CART_FAILURE'
// CREATE_SECTION
export const CREATE_SECTION = 'CREATE_SECTION'
export const CREATE_SECTION_SUCESS = 'CREATE_SECTION_SUCESS'
export const CREATE_SECTION_FAILURE = 'CREATE_SECTION_FAILURE'
// DELETE_SECTION
export const DELETE_SECTION = 'DELETE_SECTION'
export const DELETE_SECTION_SUCESS = 'DELETE_SECTION_SUCESS'
export const DELETE_SECTION_FAILURE = 'DELETE_SECTION_FAILURE'
// FETCH_ONE_SECTION
export const FETCH_ONE_SECTION = 'FETCH_ONE_SECTION'
export const FETCH_ONE_SECTION_SUCESS = 'FETCH_ONE_SECTION_SUCESS'
export const FETCH_ONE_SECTION_FAILURE = 'FETCH_ONE_SECTION_FAILURE'
// EDIT_SECTION
export const EDIT_SECTION = 'EDIT_SECTION'
export const EDIT_SECTION_SUCESS = 'EDIT_SECTION_SUCESS'
export const EDIT_SECTION_FAILURE = 'EDIT_SECTION_FAILURE'
// FETCH_ONE_LECTURE
export const FETCH_ONE_LECTURE = 'FETCH_ONE_LECTURE'
export const FETCH_ONE_LECTURE_SUCESS = 'FETCH_ONE_LECTURE_SUCESS'
export const FETCH_ONE_LECTURE_FAILURE = 'FETCH_ONE_LECTURE_FAILURE'
// CREATE_LECTURE
export const CREATE_LECTURE = 'CREATE_LECTURE'
export const CREATE_LECTURE_SUCESS = 'CREATE_LECTURE_SUCESS'
export const CREATE_LECTURE_FAILURE = 'CREATE_SECTION_FAILURE'
// DELETE_LECTURE
export const DELETE_LECTURE = 'DELETE_LECTURE'
export const DELETE_LECTURE_SUCESS = 'DELETE_LECTURE_SUCESS'
export const DELETE_LECTURE_FAILURE = 'DELETE_SECTION_FAILURE'
// EDIT_LECTURE
export const EDIT_LECTURE = 'EDIT_LECTURE'
export const EDIT_LECTURE_SUCESS = 'EDIT_LECTURE_SUCESS'
export const EDIT_LECTURE_FAILURE = 'EDIT_LECTURE_FAILURE'
// CREATE_ORDER
export const CREATE_ORDER = 'CREATE_ORDER'
export const CREATE_ORDER_SUCESS = 'CREATE_ORDER_SUCESS'
export const CREATE_ORDER_FAILURE = 'CREATE_ORDER_FAILURE'
// FETCH_ONE_ORDER
export const FETCH_ONE_ORDER = 'FETCH_ONE_ORDER'
export const FETCH_ONE_ORDER_SUCESS = 'FETCH_ONE_ORDER_SUCESS'
export const FETCH_ONE_ORDER_FAILURE = 'FETCH_ONE_ORDER_FAILURE'
// FETCH_ORDER
export const FETCH_ORDER = 'FETCH_ORDER'
export const FETCH_ORDER_SUCESS = 'FETCH_ORDER_SUCESS'
export const FETCH_ORDER_FAILURE = 'FETCH_ORDER_FAILURE'
// CONFIRM_ORDER
export const CONFIRM_ORDER = 'CONFIRM_ORDER'
export const CONFIRM_ORDER_SUCESS = 'CONFIRM_ORDER_SUCESS'
export const CONFIRM_ORDER_FAILURE = 'CONFIRM_ORDER_FAILURE'
// FETCH_IMAGE
export const FETCH_IMAGE = 'FETCH_IMAGE'
export const FETCH_IMAGE_SUCESS = 'FETCH_IMAGE_SUCESS'
export const FETCH_IMAGE_FAILURE = 'FETCH_IMAGE_FAILURE'
